...
#define VERSION "v1.4.2 Jul/22/2026"     /* Software version */
...
int *location;			/* Location inside the image */
...
int optimize_two_high(int x, int y, int color_foreground, int color_background, int reverse, int *yo)
{
    ...
    c = number_bitmaps;
    location[number_bitmaps] = x | (y << 16);
    memcpy(&bitmaps[number_bitmaps * 8], &bit[0], 8);
    number_bitmaps++;
    location[number_bitmaps] = x | ((y + 8) << 16);
...
/*
 ** Detects if a valid 4x4 solid block is found
 */
int is_valid_4x4(int x, int y) {
    int c;
    int x1;
    int y1;
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3) {
        ...
        fprintf(stderr, "    -h2    Comment each bitmap with original x,y coordinate.\n");
        ...
        fprintf(stderr, "If you add the plus sign (for example -s0000+) it will try to\n");
        fprintf(stderr, "to replace solid 4x4 blocks with Colored Squares cards\n");
        fprintf(stderr, "(allowing 4 colors per 8x8 block).\n\n");
    ...
    while (arg < argc && argv[arg][0] == '-') {
        ...
        if (c == 'f') {
            ...
            stack_color = 1;
            d = (int) strlen(argv[arg]);
            if (d != 6 && (d != 7 || argv[arg][6] != '+')) {
                ...
                if (argv[arg][6] == '+')
                    stack_color = 2;    /* Plus colored-squares mode */
            }
    ...
    if (wants_all) {
        ...
        location = malloc((size_x_cards * size_y_cards + pad_cards / 2) * sizeof(int));
    } else {
        bitmaps = malloc((max_gram + pad_cards / 2) * 8 * sizeof(char));
        location = malloc((max_gram + pad_cards / 2) * sizeof(int));
    }
    if (bitmaps == NULL || location == NULL) {
    ...
    while (1) {     /* Process each 8x8 block */
        ...
        /* Per mode */
        if (stack_color == 2) { /* Handle colored-squares mode */
            c = 1;
            buffer[0] = is_valid_4x4(x, y);
            buffer[1] = is_valid_4x4(x + 4, y);
            ...
            if (buffer[0] == stack[current_stack]
                ...
                if (buffer[1] >= 7) {
                    if (buffer[1] != stack[current_stack])
                        c = 0;
                    else
                        buffer[1] = 7;
                }
                if (buffer[2] >= 7) {
                    if (buffer[2] != stack[current_stack])
                        c = 0;
                    else
                        buffer[2] = 7;
                }
                if (buffer[3] >= 7) {
                    if (buffer[3] != stack[current_stack])
                        c = 0;
                    else
                        buffer[3] = 7;
                }
            ...
            if (c) {
                *ap++ = buffer[0] | (buffer[1] << 3) | (buffer[2] << 6) | ((buffer[3] & 3) << 9) | ((buffer[3] & 4) << 11) | 0x1000;
            }
        } else {
            c = 0;
        }
    ...
    if (intybasic == 1) {  /* IntyBASIC mode */
        ...
        for (c = 0; c < number_bitmaps; c++) {
            ...
            if (use_bitmap) {
                if (show_location && location[c] != -1) {
                    fprintf(a, "\tBITMAP \"%s\"\t' %d,%d\n", binary(bitmaps[c * 8 + 0]), location[c] & 0xffff, location[c] >> 16);
                } else {
        ...
        for (c = 0; c < number_bitmaps; c++) {
            if (show_location && location[c] != -1) {
                if (decle8)
                    fprintf(a, "\tDECLE $%02X,$%02X,$%02X,$%02X,$%02X,$%02X,$%02X,$%02X\t; %d,%d\n",
